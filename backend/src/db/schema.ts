...
export const usersRelations = relations(users, ({ many }) => ({
  products: many(products), // one user -> many products 1:N
  comments: many(comments), // one user -> many comments 1:N
}));

// Products Relations: a product belongs to one user and can have many comments
// `one()` means single related record, `many()` means multiple related records
...
export const productsRelations = relations(products, ({ one, many }) => ({
  ...
  user: one(users, { fields: [products.userId], references: [users.id] }), // 1:1
}));
